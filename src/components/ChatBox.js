  ...
  return (
    ...
    >
      ...
      <div className="w-full h-[91%]">
        ...
        <div className="w-full h-[90%] flex-col justify-between relative overflow-y-auto px-4">
          {
            selectedChat.chats?.map((chat, index) => {
              if (!chat.you) {
                return (
                  <div className="text-white h-auto rounded-b-lg rounded-tr-lg px-1 mt-12 mr-3 bg-primary relative items-center flex-col py-1 group max-w-xl w-max min-w-[150px]">
                    <MenuTextMsgL menuTextL={menuTextL} />
                    {selectedChat.isGroupChat ? (<p className="text-sm px-2 max-w-full break-words text-pink-400">{chat.name}</p>) : ("")}
                    <p className="text-sm px-2 max-w-full break-words">{chat.message}</p>
                    ...
                    <p className="text-icon text-xs mt-auto mr-1 ml-auto w-max">{chat.date} -- {chat.time}</p>
                ...
                return (<div className="text-white h-auto rounded-b-lg rounded-tl-lg px-1 mt-5 mr-3 bg-[#008069] relative items-center flex-col py-1 group max-w-xl w-max ml-auto min-w-[150px]">
                  <MenuTextMsg menuText={menuText} setMenuText={setMenuText} />
                  <p className="text-sm px-2">{chat.message}</p>
